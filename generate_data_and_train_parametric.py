...
def parse_args():
    parser = argparse.ArgumentParser(description='Train scene classifier with hyperparameter search')
    parser.add_argument('--lr', type=float, default=2e-4, help='Learning rate')
    parser.add_argument('--epochs', type=int, default=5, help='Number of epochs')
    parser.add_argument('--lora_r', type=int, default=16, help='LoRA rank')
    parser.add_argument('--exp_name', type=str, default='baseline', help='Experiment name for output paths')
...
LORA_R = args.lora_r  # LoRA rank (from CLI)
LORA_ALPHA = args.lora_r * 2  # LoRA alpha (keep 2:1 ratio)
LORA_DROPOUT = 0.1  # LoRA dropout
USE_4BIT = True  # Use 4-bit quantization (QLoRA) - reduces memory for single GPU

# Training
BATCH_SIZE = 32  # Original 3B recipe batch size
LEARNING_RATE = args.lr  # Learning rate (from CLI)
...
if CLASSIFIER_TYPE == "llm-finetune":
    ...
    # Include experiment name in path for grid search
    CLASSIFIER_OUTPUT = f"data/models/llm-3b_{args.exp_name}"
    PLOT_OUTPUT = f"plots/llm-3b_{args.exp_name}_training.png"
else:
...
def main():
    ...
    if CLASSIFIER_TYPE == "llm-finetune":
        ...
        # Training arguments - Parametric for grid search
        training_args = TrainingArguments(
            output_dir=CLASSIFIER_OUTPUT,
            num_train_epochs=args.epochs,  # From CLI
            per_device_train_batch_size=16,  # Adjusted for single H100
            per_device_eval_batch_size=16,
            gradient_accumulation_steps=1,
            learning_rate=LEARNING_RATE,  # From CLI (via args.lr)
            weight_decay=WEIGHT_DECAY,  # 1e-5 from config
            warmup_steps=100,  # Original 3B recipe
            logging_dir=f"{CLASSIFIER_OUTPUT}/logs",
            logging_steps=50,
            eval_strategy="steps" if val_dataset else "no",  # Eval every 500 steps
            eval_steps=500,
            save_strategy="steps",
            save_steps=500,
